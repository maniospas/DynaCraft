...
class PythonGenerator:
    ...
    def init_float_var(self, tab):
        ...
        initialization = f"{tab}{variable} = {expression}"
        return initialization

    def init_method_float_var(self, used_objects, local_free_variables,local_used_variables, tab):
    ...
    def init_object(self, exclude_obj = "fun_10", tab=""):
        obj_name = random.choice(self.variables)
        result = f"{tab}{obj_name} = dict() "
    ...
    def generate_method_body(self, fun_name = "fun6", tab="", in_method=False):
        method_lines = []
        used_objects = []
        ...
        if in_method:
            ...
            return_line = f"{tab}return {return_var}"
            method_lines.append(return_line)

        return method_lines


    def generate_method(self, tab=""):
        ...
        method_body = '\n'.join(self.generate_method_body(method_name, tab=tab+'   ', in_method=True))
        result = f"{tab}def {method_name}({params_string}):\n{method_body}"

        return result


    def generate_methodcall(self, tab=""):
        ...
        result = f"{tab}{variable} = {method_call_str}"
    ...
    def generate_if_statement(self, tab=""):
        if random.choice([True, False]):
            comparison = self.generate_comparisson()
        else:
            comparison = self.method_call_helper(random.choice(self.used_functions))
        body_lines1 = self.generate_method_body(tab=tab+'   ')
        codeblock1 = "\n".join(f"{line}" for line in body_lines1)
        body_lines2 = self.generate_method_body(tab=tab+'   ')
        codeblock2 = "\n".join(f"{line}" for line in body_lines2)

        result = f"if {comparison}:\n{codeblock1}\nelse:\n{codeblock2}"
        return result


    def while_statement(self, tab=""):
        if random.choice([True, False]):
            comparison = self.generate_comparisson()
        else:
            comparison = self.method_call_helper(random.choice(self.fun_names))
        body_lines = self.generate_method_body(tab=tab+'   ')
    ...
    def generic_switch_case(self, value, tab=""):
        switcher = {
        ...
        return switcher.get(value)(tab=tab)

    def method_switch_case(self, value, used_objects, exlude_function="fun_10", local_free_varianbles=[], local_used_variables=[], tab=""):
        switcher = {
            1: lambda tab:self.init_method_float_var(used_objects, local_free_varianbles, local_used_variables, tab=tab),
            2: lambda tab:self.init_object(exlude_function, tab=tab),
